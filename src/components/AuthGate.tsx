...
// ─── User context — accessible from any child component ──────────────────────
...
export default function AuthGate({ children }: Props) {
  ...
  useEffect(() => {
    ...
    getRedirectResult(_auth)
      .then(result => {
        if (!result) return;
        // Any Google account is fine — just set the user
      })
      .catch(e => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0a0f1e] px-6">
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-slate-500 mt-1">Googleアカウントでログインしてください</p>
        </div>

        {errMsg && (
          <div className="mb-6 bg-red-900/30 border border-red-500/40 rounded-2xl px-4 py-3 text-xs text-red-300 text-center max-w-xs">
            {errMsg}
          </div>
        )}
  ...
  // Authenticated — provide user via context
  return (
